    ...
    <section className="gradient-hero relative overflow-hidden border-b border-border/60 bg-gradient-to-b from-background via-background to-[hsl(var(--surface))] py-20 lg:py-28">
      ...
      <div className="container relative">
        ...
        <div className="mx-auto max-w-4xl text-center">
          ...
          <h1 className="mt-6 text-balance font-extrabold tracking-tight text-foreground text-hero md:text-5xl lg:text-display">
            {title}
          </h1>

          <p className="mx-auto mt-5 max-w-3xl text-balance text-body leading-relaxed text-muted-foreground md:text-lg">
            {description}
          </p>
          ...
          {trustPoints && trustPoints.length > 0 && (
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              {trustPoints.map((point) => (
                <div
                  key={point}
                  className="inline-flex items-center gap-2 rounded-full border border-border/80 bg-card/90 px-4 py-2.5 text-small text-foreground shadow-card"
                >
