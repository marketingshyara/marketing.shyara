    ...
    <Layout>
      ...
      <section className="py-10 bg-[hsl(var(--surface))]">
        <div className="container">
          <div className="max-w-5xl mx-auto rounded-card border border-accent/30 bg-gradient-to-br from-accent/5 to-accent/10 p-6 shadow-card">
            <div className="mb-4">
              <h2 className="text-body font-semibold text-accent">Core Value Across All Plans</h2>
            </div>
      ...
      <section className="py-20 lg:py-28 bg-background">
        <div className="container">
          <div className="text-center mb-16">
            ...
            <h2 className="text-section font-bold text-foreground">Simple. Consistent. Clear.</h2>
          </div>
      ...
      <section className="py-20 lg:py-28 bg-[hsl(var(--surface))]">
        <div className="container">
          <div className="text-center mb-12">
            ...
            <h2 className="text-section font-bold text-foreground">Choose Your Plan</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto">
            {plans.map((plan) => (
              <div
                key={plan.name}
                className={`relative rounded-card border p-6 flex flex-col shadow-card ${
                  plan.popular ? "border-accent bg-accent/5 shadow-elevated" : "border-border bg-card"
                }`}
              >
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-accent text-accent-foreground text-caption font-medium px-3 py-1 rounded-lg">
                    Most Popular
                  </div>
                )}
                <h3 className="text-lg font-semibold text-foreground">{plan.name}</h3>
                <div className="mt-2 mb-4">
                  <span className="text-section font-bold text-foreground">{plan.price}</span>
                  <span className="text-muted-foreground">{plan.period}</span>
                </div>
                <p className="text-small text-muted-foreground mb-5">{plan.description}</p>
          ...
          <div className="mt-14 max-w-4xl mx-auto">
            <h3 className="text-section font-semibold text-center mb-6 text-foreground">Plan Comparison</h3>
      ...
      <section className="py-16 bg-background">
        <div className="container max-w-3xl mx-auto text-center">
          <div className="rounded-card border border-accent/30 bg-accent/5 p-6 shadow-card">
      ...
      <section className="py-20 lg:py-28 bg-[hsl(var(--surface))]">
        <div className="container max-w-3xl mx-auto">
          ...
          <div className="space-y-3">
            {faqItems.map((item, i) => (
              <div key={i} className="rounded-card border border-border bg-card overflow-hidden shadow-card focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:ring-offset-background">
                <button
                  type="button"
                  className="w-full flex items-center justify-between px-6 py-4 text-left gap-4 focus:outline-none focus-visible:ring-0"
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  aria-expanded={openFaq === i}
                >
                  <span className="font-medium text-foreground text-small">{item.question}</span>
                  <ChevronDown
                    className={`h-4 w-4 text-accent flex-shrink-0 transition-transform duration-200 ${openFaq === i ? "rotate-180" : ""}`}
                  />
                </button>
                {openFaq === i && (
                  <div className="px-6 pb-4 text-small text-muted-foreground leading-relaxed border-t border-border pt-3">
                    {item.answer}
                  </div>
                )}
              </div>
      ...
      <section className="py-20 bg-ctaBand border-l-4 border-accent text-white">
        <div className="container max-w-3xl mx-auto text-center">
          <h2 className="text-section font-bold mb-2">Ready to Get Started?</h2>
          <p className="text-white/70 text-small mb-6">Choose a plan above or talk to us to find the right fit.</p>
          <Button
            size="lg"
